    ...
    case "add":
        if (HavePermission("SubjectGroup", "ADD")) {
            if (!isset($_POST["submit"])) {
                ...
                } elseif (IsSubjectOwner() != false) {
                    new GroupAddView(IsSubjectOwner());
                } else {
                    new GroupAddView([]);
                }
            ...
            try {
                ...
                if (!isset($_POST["submit"])) {
                    if (IsAdmin()) {
                        ...
                        new GroupEditView($group, IsSubjectOwner());
                    } else {
                        new GroupEditView($group, $group->getSubject());
                    }
        ...
        try {
            ...
            if (!$break) {
                ...
                if (!empty($sub) && count($sub) == 1) {
                    ...
                    new GroupShowAllView($groupsData, $itemsPerPage, $currentPage, $totalGroups, $toSearch,$searching,$permission);
                } elseif (count($sub) > 1) {
                    $groupsData = array();
                    foreach ($sub as $s) {
                        $group->setSubject($sub[0]);
                        ...
                        $toSearch = getToSearch($search);
                        $totalGroups += $GLOBALS["groupDAO"]->countTotalGroups($toSearch);
                        $data = $GLOBALS["groupDAO"]->showAllPaged($currentPage, $itemsPerPage, $toSearch);
                        foreach ($data as $dat) {
                            array_push($groupsData, $dat);
                        }
                    }
                    new GroupShowAllView($groupsData, $itemsPerPage, $currentPage, $totalGroups, $toSearch,$searching,$permission);
                } else {

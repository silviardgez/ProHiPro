...
class UniversityDAO
{
    ...
    function show($key, $value)
    {
        ...
        $user = $this->userDAO->show("login", $university["user_id"]);
        return new University($university["id"], $academicCourse, $university["name"], $user);
    }
    ...
    private function getUniversitiesFromDB($universitiesDB)
    {
        ...
        foreach ($universitiesDB as $university) {
            $academicCourse = $this->academicCourseDAO->show("id", $university["academic_course_id"]);
            $user = $this->userDAO->show("login", $university["user_id"]);
            array_push($universities, new University($university["id"], $academicCourse, $university["name"],$user));
        }
        return $universities;
    }
}
...
class PermissionEditView {
    ...
    function __construct($permission, $roleData, $funcActionData){
    ...
    function render(){
        ...
        <main role="main" class="margin-main ml-sm-auto px-4">
            ...
            border-bottom">
                <h1 class="h2" data-translate="Editar asignación de permiso"></h1>
                    ...
                    <select class="form-control" id="role_id" name="role_id">
                        <?php foreach ($this->roles as $role): ?>
                            <option value="<?php echo $role->getId()?>"
                                <?php if($role->getId() == $this->permission->getRole()->getId()){echo 'selected="selected"';}?>>
                    ...
                    <select class="form-control" id="func_action_id" name="func_action_id">
                        <?php foreach($this->funcActions as $funcAction): ?>
                            <option value="<?php echo $funcAction->getId()?>"
                                <?php if($funcAction->getId() == $this->permission->getFuncAction()->getId()){echo 'selected="selected"';}?>>
                                <?php echo $funcAction->getAction()->getName()." - ".$funcAction->getFunctionality()->getName(); ?>
                            </option>
                        <?php endforeach;?>
                    </select>
                </div>
